...
def homepage_update():
    ...
    with open(homepage_path, "r") as yaml_file:
        data = yaml.safe_load(yaml_file)
    
    response = requests.get('http://web1.elemento.cloud:8080/api/http/routers') #ToDo: generalize the URL of traefik
    routers = response.json()
    for router in routers:
        name = router["name"].split("@")[0]
        if is_custom_service(router["rule"]) and name not in data[0]["Services"] and name != "homepage":
          icon = icons[name] if name in icons else ""
          data[0]["Services"].append({name: {"icon": icon,"href": router["rule"].split('`')[1]}})
